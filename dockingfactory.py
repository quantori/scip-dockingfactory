...
def func_docking(ligand):
    try:
        ...
        if variables['handler'] != "rdock":
            ...
            docking.set_ligand(ligand)
            docking.set_rdockprm(variables['rdock_protocol'])
            docking.set_rdocksys(variables['rdock_recdef'])
            docking.set_rdockroot(variables['rdock_root'])
            docking.set_rdockhome(variables['rdock_home'])
            docking.set_nruns(variables['rdock_nruns'])
        ...
        if not variables['server_mode'] and current == total and len(chunks_done) == 0 \
            ...
            if errors_aws != 0:
                ...
                if variables['handler'] != "rdock":
                    ...
                    files = get_sd_from_folder(variables['input_path'])
                done = []
                ...
                if isinstance(result[0], Exception):
                    ...
                    if variables['handler'] != "rdock":
                        ...
                        if 'output_folder' in variables:
                            ...
                            file = getNameisExist(file)
                            csv = open(variables['csv_out'], "a")
                            csv.write("{},{},{}\n".format(result[1], result[0][1], file))
                            csv.close()

                            f = open(file, "w")
                            f.write(f"{result[0][2]}")
                            f.close()
    ...
    while True:
        ...
        if not variables['local']:
            if 'cost_per_cpu' in variables:
                ...
                if lig_per_sec:
                    eta_cost = current_cost + (total - current) / lig_per_sec * (
                                variables['cost_per_cpu'] + variables['cost_per_cpu'] * num_cpus) / 3600
                print(
                    '%d/%d/%d/%d clusters: %d, workers: %d, CPUs: %d, Elapsed: %d min %02d s, Left: %d min, Rate: %.2f lig/sec, Current cost: $%.2f, Cost estimate: $%.2f' % (
                    current, errors, errors_aws, total, num_clusters, num_workers, num_cpus, elapsed // 60,
                    elapsed % 60, time_left, lig_per_sec, current_cost, eta_cost))
            else:
                print(
                    '%d/%d/%d/%d clusters: %d, workers: %d, CPUs: %d, Elapsed: %d min %d s, Left: %d min, Rate: %.2f lig/sec' % (
                    current, errors, errors_aws, total, num_clusters, num_workers, num_cpus, elapsed // 60,
                    elapsed % 60, time_left, lig_per_sec))
        else:
            print('%d/%d/%d/%d, Elapsed: %d min %d s, Left: %d min, Rate: %.2f lig/sec' % (
            current, errors, errors_aws, total, elapsed // 60, elapsed % 60, time_left, lig_per_sec))
        await asyncio.sleep(1)
...
def get_sd_from_folder(path):
    return (list(map(lambda x: str(x.absolute()), list(Path(path).rglob("*.sd")))))
...
async def main():
    ...
    if variables['handler'] != "rdock":
        ...
        filenames = get_sd_from_folder(variables['input_path'])
    ...
    if not variables['local']:
        tasks = [asyncio.ensure_future(metacluster()), asyncio.ensure_future(scheduler()),
                 asyncio.ensure_future(monitor()), asyncio.ensure_future(results_handler())]
        try:
            await asyncio.gather(*tasks)
        except Exception:
            for t in tasks:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_path", type=str,
                        help="Path to folder with ligands (.pdbqt, subdirectories are searched automatically)")
    ...
    parser.add_argument("--rdock_protocol", type=str, default="dock.prm", help="Protocol for rDock (.prm)")
    parser.add_argument("--rdock_recdef", type=str, help="Receptor definition for rDock (.prm)")
    parser.add_argument("--rdock_nruns", type=int, help="Number of runs for rdock")
    parser.add_argument("--rdock_root", type=str, help="Root directory of rDock files")
    parser.add_argument("--rdock_home", type=str, help="Home directory of rDock files")
    ...
    parser.add_argument("--error_msg_out", type=str,
                        help="Output file containing error messages that come from docking engine (.txt)")
    parser.add_argument("--errors_aws", type=str,
                        help="Output file containing error messages that come from AWS (.txt)")
    ...
    if 'output' in variables:
        ...
        if [i for i in ['csv_out', 'failed_ligand_out', 'error_msg_out', 'output_folder'] if i in variables]:
            raise Exception(
                "Incorrect config: use either 'output' or ['csv_out', 'failed_ligand_out', 'error_msg_out', 'output_folder']")
        if not variables['restart']:
            if not Path(variables['output']).exists() or len(os.listdir(variables['output'])) > 1 or \
                    (len(os.listdir(variables['output'])) == 1 and os.listdir(
                        f"{variables['output']}/{os.listdir(variables['output'])[0]}")):
    ...
    if not variables['local']:
        ...
        cluster_conf = dict()
        for conf in ['address', 'maximum_scale', 'name', 'partition', 'worker_instance_type',
